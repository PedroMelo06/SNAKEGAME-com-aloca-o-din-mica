...
typedef struct {
    Posicao* corpo;  // Ponteiro para alocar dinamicamente o corpo da cobra
    int tamanho;
} Cobra;
...
void jogarCobrinha(int nivel){
    Cobra cobra;
cobra.corpo = (Posicao*)malloc(sizeof(Posicao) * (38 * 13));  // Aloca memória dinamicamente para o corpo da cobra
if (cobra.corpo == NULL) {
    printf("Erro ao alocar memória para o corpo da cobra!\n");
    exit(1);  // Se não conseguir alocar, o jogo não pode continuar
}
    ...
    int ch; // c?digo da tecla pressionada
    ...
    while (ch != KEY_ESC){
        ...
        // Verificar o c?digo da tecla pressionada
        switch (ch){
            case ARROW_UP:
                cobra.corpo[0].y--;
                break;
        ...
        if(ponto.ponto.x == cobra.corpo[0].x && ponto.ponto.y == cobra.corpo[0].y){   
            ...
            cobra.corpo = (Posicao*)realloc(cobra.corpo, sizeof(Posicao) * tamanhoCobra);
    if (cobra.corpo == NULL) {
        printf("Erro ao realocar memória para o corpo da cobra!\n");
        exit(1);  // Se não conseguir realocar, o jogo não pode continuar
    }
            pontos++; // aumentar os pontos
            // Gerar o pr?ximo ponto
            ponto.ponto.x = (rand() % 37) + 1;
            ponto.ponto.y = (rand() % 10) + 1;
        }
     ...
     // Liberar memória alocada para o corpo da cobra
    free(cobra.corpo);
